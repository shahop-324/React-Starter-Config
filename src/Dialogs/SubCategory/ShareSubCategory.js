...
const ShareSubCategory = ({ open, handleClose, id }) => {
  ...
  const link = `qwikshop.online/${store.subName}/product/search/category/${name}`;

  return (
    <>
      <Dialog fullWidth maxWidth="sm" open={open}>
        <DialogTitle>Share sub category</DialogTitle>
              ...
              title={
                store.shareProductMessage
                  ? store.shareProductMessage
                      ...
                      .replace(/{{subCategoryName}}/g, name)
                      .replace(/{{subCategoryLink}}/g, link)
                  : name
              }
              separator=":"
            >
...
export default ShareSubCategory;
