...
import { bulkUpdateSubCategories, resetIsBulkUpdatingSubCategories } from '../../actions';
...
const BulkUpdateSubCategory = ({ open, handleClose }) => {
  const dispatch = useDispatch();
  const [rows, setRows] = useState([]);

  useEffect(() => {
    dispatch(resetIsBulkUpdatingSubCategories());
  }, []);

  const { subCategories, isBulkUpdating } = useSelector((state) => state.subCategory);

  useEffect(() => {
    const mRows = subCategories
      .map((el) =>
        Object.keys(el)
  ...
  return (
    <>
      <Dialog fullWidth maxWidth="sm" open={open}>
        <DialogTitle>Bulk Update Sub Category</DialogTitle>
        <Stack sx={{ p: 3 }} direction="row" alignItems="center" justifyContent="space-between">
          ...
          <div className="d-flex flex-row align-items-center justify-content-end">
            <LoadingButton
              ...
              onClick={() => {
                dispatch(bulkUpdateSubCategories(rows, handleClose));
              }}
              type="button"
              variant="contained"
            >
...
export default BulkUpdateSubCategory;
