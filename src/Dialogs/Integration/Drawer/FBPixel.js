...
const FBPixelConnect = ({ open, handleClose }) => {
  const dispatch = useDispatch();
  const { store } = useSelector((state) => state.store);

  const [pixelId, setPixelId] = useState(store.FacebookPixelId);
        ...
        <Drawer anchor={'right'} open={open} onClose={handleClose}>
          <Box sx={{ my: 3, mx: 4, width: '400px' }}>
            ...
            <Box sx={{ my: 4 }}>
              ...
              <Button
                onClick={() => {
                  dispatch(updatePixel(pixelId, handleClose, false));
                }}
                sx={{ my: 2 }}
                variant="contained"
                fullWidth
              ...
              <Link to="/">
                <Typography variant="caption">See How to obtain Facebook Pixel ID</Typography>
              </Link> */}
            </Box>
          </Box>
        </Drawer>
