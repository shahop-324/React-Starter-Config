...
import { bulkUploadReferral, resetIsBulkImportingReferral } from '../../actions';
...
const columns = [
  ...
  },
  {
    field: 'phone',
    headerName: 'Phone',
    description: 'Referral mobile number',
    width: 200,
    editable: true,
    sortable: true,
  },
  {
    field: 'email',
    headerName: 'Email',
    description: 'Referral email address',
    width: 200,
    editable: true,
    sortable: true,
  },
...
const BulkUploadReferral = ({ open, handleClose }) => {
  const [rows, setRows] = useState([]);
  const [invalidFormat, setInvalidFormat] = useState(false);
  const { isBulkImporting } = useSelector((state) => state.referral);

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(resetIsBulkImportingReferral());
  }, []);
  ...
  return (
    <>
      <Dialog fullWidth maxWidth="md" open={open}>
        <DialogTitle>Bulk Upload Referral</DialogTitle>
              ...
              <Card sx={{ p: 3 }}>
                <div>
                  <Stack sx={{ mb: 3 }} direction={'row'} alignItems="center" justifyContent={'space-between'}>
                    <LabelStyle>Import Referrals via Excel / CSV</LabelStyle>
                    <div className="d-flex flex-row align-items-center justify-content-end">
                      <LoadingButton
                        loading={isBulkImporting}
                        disabled={!(rows !== undefined && rows.length > 0)}
                        onClick={() => {
                          dispatch(bulkUploadReferral(rows, handleClose));
                        }}
                    ...
                    <Card sx={{ p: 4 }}>
                      <Stack direction={'column'} alignItems="center" justifyContent={'center'} spacing={3}>
                        ...
                        <Typography variant="body2" color={'#538bF7'}>
                          NOTE: Only name, phone, email & commission fields are required
                        </Typography>
                        <a
                          style={{ textDecoration: 'none' }}
                          target={'_blank'}
...
export default BulkUploadReferral;
